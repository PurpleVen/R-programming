...
shinyServer(function(input, output) {
  ...
  bb_data <- read.csv("C:/AICTE Project/GIS Project/Converted Files/Dustbin.csv", stringsAsFactors = FALSE )
  ...
  dd_data <- read.csv2("C:/AICTE Project/GIS Project/Converted Files/Landmarks.csv")
  ...
  output$plot <- renderPlot({
    sales <- read.csv("C:/AICTE Project/GIS Project/Converted Files/Dustbin.csv", header = TRUE, sep = ",")
    View(sales)
    #Plot
    x<- table(sales$Zone)
    barplot(x,xlab ="Zone",ylab = "Dustbins" , main = "Dustbin Availability On Different Zones Of Varanasi")
